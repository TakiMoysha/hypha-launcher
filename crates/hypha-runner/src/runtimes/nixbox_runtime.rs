...
impl Default for NixboxRuntime {
    fn default() -> Self {
        Self {
            ...
            cgroup_path: Some(PathBuf::from("/sys/fs/cgroup").join(gen_cgroup_name(&"default"))),
            cgroup_limits: CgroupLimitsOpts {
                max_memory: 2048,
                max_pids: 1024,
            },
...
impl Drop for NixboxRuntime {
    fn drop(&mut self) {
        ...
        if let Some(path) = &self.cgroup_path {
            path.exists()
                .then(|| fs::remove_dir_all(path).expect("[ERROR] Failed to remove cgroup dir"));
        }

        if let Some(path) = &self.container_dir {
            path.exists()
                .then(|| fs::remove_dir_all(path).expect("[ERROR] Failed to remove container dir"));
        }
    }
}
